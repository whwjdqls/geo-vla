...
class Config:
    ...
    aux_dim: int = 0  # for auxiliary experts
...
def get_config(variant: Variant) -> Config:
    ...
    if variant == "point_head_v1":
        ...
        config =  Config(
            ...
            aux_dim=512 * 3, # this must be divisible by 3
        )
        assert config.aux_dim % 3 == 0, "aux_dim must be divisible by 3"
        return config
    if variant == "point_head_v2":
        # 31M params
        config =  Config(
            width=1024,
            depth=6,
    ...
    if variant == "point_head_v3": # this is the one that uses 1024 full point clouds
        ...
        config =  Config(
            ...
            num_kv_heads=1,# this must be matched
            head_dim=256, # this must be matched I think
            aux_dim=1024 * 3, # this must be divisible by 3
        )
        assert config.aux_dim % 3 == 0, "aux_dim must be divisible by 3"
        return config
    if variant == "depth_head_v1":
        return Config(
            ...
            num_heads=8, # this must be matched
            num_kv_heads=1,# this must be matched
            head_dim=256, # this must be matched 
            aux_dim=16*16*4, # this must be matched
        )
    if variant == "depth_head_v2":
        return Config(
            width=1024,
            depth=6,
            mlp_dim=2048,
