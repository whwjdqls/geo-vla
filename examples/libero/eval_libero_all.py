...
class Args:
    ...
    # Robustness
    #################################################################################################################
    reconnect_on_fail: bool = True  # reconnect websocket if infer fails
...
def eval_libero(args: Args) -> None:
    ...
    # print if saving videos
    if args.save_videos:
        logging.info("Videos will be saved.")
    else:
        logging.info("Videos will NOT be saved.")
    # Output paths
        ...
        for p in [metrics_json_path, metrics_csv_path, summary_json_path]:
            if p.exists():
                raise FileExistsError(
                    f"{p} already exists. Set --overwrite True or change --out-dir."
                )
                ...
                try:
                    ...
                    if not action_plan:
                        ...
                        try:
                            ...
                            # Optionally reconnect and retry once
                            if args.reconnect_on_fail:
                                logging.error("infer failed; reconnecting client once. error=%s", e)
                                client = _websocket_client_policy.WebsocketClientPolicy(args.host, args.port)
                                action_chunk = client.infer(element)["actions"]
                            else:
                                raise
                    ...
                    logging.error("Caught exception (task_id=%d, episode=%d, t=%d): %s", task_id, episode_idx, t, e)
                    break
