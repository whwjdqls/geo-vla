...
class RobocasaInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        # NOTE that we have 2 base images and one wrist image for RoboCasa dataset.
        inputs = {
        ...
        if "point_cloud" in data:
            inputs["point_cloud"] = data["point_cloud"]
        # Depth can arrive either as raw frames (depth_image) or precomputed latents (depth_latent).
        # Downstream code expects the key name "depth_image".
        if "depth_image" in data:
            inputs["depth_image"] = data["depth_image"]
        elif "depth_latent" in data:
            inputs["depth_image"] = data["depth_latent"]
